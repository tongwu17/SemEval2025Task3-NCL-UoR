...
model = AutoModelForCausalLM.from_pretrained(model_name, trust_remote_code=True, cache_dir="./tmp").to(device)
tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True, cache_dir="./tmp")
...
file_path = "multiparallel-fi-with-questions-filtered.tsv"
...
for shorthand, config in tqdm.tqdm(configs):
    output_file_path = f'outputs/fi-{model_name.split("/")[1]}.{shorthand}.jsonl'
    annotation_file_path = f'outputs/fi-{model_name.split("/")[1]}-annotation.{shorthand}.jsonl'
    if not pathlib.Path(annotation_file_path).is_file():
        ...
        with open(output_file_path, 'w', encoding='utf-8') as file:
            for record in tqdm.tqdm(records):
                ...
                response = outputs.sequences[0][input_length:] # [inputs.shape[-1]:]
