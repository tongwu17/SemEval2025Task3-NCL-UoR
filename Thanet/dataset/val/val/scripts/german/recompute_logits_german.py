...
def apply_model_specific_templtate(mname, ans, tokenizer, vocab_lookup=None):
    if 'occiglot' in mname:
        ...
        all_tokens = prompt_tokens + tokenizer.convert_tokens_to_ids(ans.output_tokens)
        if tokenizer.eos_token is not None and all_tokens[-1] != tokenizer.eos_token_id:
            all_tokens.append(tokenizer.eos_token_id)
        # sentence = prompt + ans.output_text + tokenizer.eos_token
        data = {'input_ids' : [all_tokens], 'attention_mask': [[1] * len(all_tokens)], }
        inputs = BatchEncoding(data, tensor_type='pt').to(device)
        #inputs = tokenizer(sentence, return_tensors="pt").to(device)
        if not (inputs['input_ids'].cpu()[0,:prompt_len] == torch.tensor(prompt_tokens)).all():
...
def main():
    ...
    outfile = 'german_ready_for_anotation_fixed2.tsv'
    ...
    # NaN's are for SauerkrautLLM (llama_cpp)
